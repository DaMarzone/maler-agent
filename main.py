...
# Gesprächsspeicher: { chat_id: [{"role": ..., "content": ...}, ...] }
gespraech = {}
...
WICHTIG - GEDAECHTNIS:
Lies den GESAMTEN Gespraechsverlauf sorgfaeltig. Extrahiere alle bereits genannten Informationen.
Frage NIEMALS nach etwas, das bereits im Verlauf erwaehnt wurde.

PFLICHTFELDER:
GRUPPE A - Auftragsdaten: mind. eine Leistung mit Mengenangabe (m2, Stueck, lfd. m)
GRUPPE B - Kundendaten: Vor- und Nachname, Strasse + Hausnummer, PLZ, Ort

VORGEHEN:
1. Extrahiere alle bereits bekannten Informationen aus dem gesamten Verlauf
2. Pruefe welche Pflichtfelder noch fehlen
3. Wenn Infos fehlen: Stelle ALLE fehlenden Fragen einer Gruppe auf einmal (nicht einzeln)
   - Zuerst fehlende Auftragsdaten (Gruppe A), dann fehlende Kundendaten (Gruppe B)
   - Formuliere die Rueckfrage als eine zusammenhaengende Nachricht
4. Wenn alle Infos vorhanden: Erstelle das Angebot

ANTWORTFORMAT:

Falls Informationen fehlen:
{{"status": "rueckfrage", "frage": "Deine Rueckfrage mit ALLEN fehlenden Punkten dieser Gruppe"}}
...
def erstelle_pdf(angebot: dict, betrieb: dict, ang_nr: str, heute: str, gueltig: str) -> BytesIO:
    ...
    kopf = Table([
        ...
        [Paragraph("", normal), Paragraph(f"Gueltig bis: {gueltig}", normal)],
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Audio oder Text — beide Pfade landen im gleichen Verlauf
    ...
    # Verlauf aufbauen — Text immer als user-Nachricht hinzufuegen
    ...
    try:
        ...
        if antwort.get("status") == "rueckfrage":
            ...
            # Antwort in Verlauf speichern damit Claude sie beim naechsten Mal kennt
        ...
        if antwort.get("status") == "angebot":
            ...
            heute = datetime.now().strftime("%d.%m.%Y")
            gueltig = (datetime.now() + timedelta(days=30)).strftime("%d.%m.%Y")
            gespraech[chat_id] = []  # Reset nach erfolgreichem Abschluss
